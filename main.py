...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        # Загружаем простые данные всех товаров для поиска (для малых/средних магазинов это быстро и надежно)
        all_products_rows = conn.execute("SELECT * FROM products").fetchall()
        all_products = [dict(r) for r in all_products_rows]
        conn.close()

        # Очистка и разбивка на слова (поддержка знаков препинания)
        import re
        clean_message = re.sub(r'[^\w\s]', ' ', user_message_lower) # Заменяем знаки препинания на пробелы
        ...
        if words:
            ...
            for p in all_products:
                ...
                for word in words:
                    # Совпадение в названии весит больше (3 балла)
                    if word in p_name_lower:
                        score += 3
                    # Совпадение в описании (1 балл)
                    elif word in p_desc_lower:
                        score += 1
            ...
            found_products = [item[1] for item in scored_products[:5]]
        
        # 2. GPT Генерация ответа
        if openai_client:
            # Формируем контекст товаров для бота
            products_context = ""
            if found_products:
                products_context = "Знайдені товари у базі:\n" + "\n".join(
                    [f"- ID {p['id']}: {p['name']} ({p['price']} грн). {p.get('description', '')[:100]}..." 
                     for p in found_products]
                )
            else:
                products_context = "Товарів за запитом не знайдено у базі."

            # Системный промпт
            system_prompt = f"""
            Ты — дружелюбный консультант магазина DikorosUA (грибы, травы, добавки).
            Твоя цель — помочь подобрать товар и продать его.
            
            Контекст каталога (то что нашлось по запросу):
            {products_context}
            
            1. Если есть подходящие товары — порекомендуй их, расскажи коротко о пользе.
            2. Если товаров нет — извинись и предложи посмотреть каталог или спросить о другом.
            3. Отвечай на языке запроса (УКР/РУС).
            4. Не используй Markdown форматирование (не пиши **жирным** или # заголовки). Пиши обычным текстом.
            5. Будь краток (макс 3-4 предложения).
            """
            
            history = [{"role": "system", "content": system_prompt}]
            # Добавляем последние 3 сообщения для контекста разговора (адаптируем роль)
